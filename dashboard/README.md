...
Les modèles (`models/*.joblib`) et le jeu de données scoré (`data/passengers_scored.parquet`) sont déjà générés et versionnés — aucune étape supplémentaire n'est nécessaire pour lancer l'app.
...
Le script régénère `models/lr_pipeline.joblib`, `models/knn_pipeline.joblib`, `data/passengers_scored.parquet`, `data/metrics.json` et `data/lr_coefficients.json`.

## Déploiement gratuit (Streamlit Community Cloud)

1. Se rendre sur [share.streamlit.io](https://share.streamlit.io) et se connecter avec le compte GitHub `kenzo-kouokam`.
2. Cliquer sur **New app**, sélectionner le dépôt `airline-satisfaction-ml`, la branche `main`, et le fichier principal `dashboard/app.py`.
3. Déployer — Streamlit installe automatiquement `dashboard/requirements.txt`.
...
├── data/                   ← Dataset scoré (parquet) + métriques précalculées (json)
